# City Mysteries App - Progress Tracker V2

## Current Phase: PHASE 6
## Current Step: 6.3

## PHASE 6: Deploy to AWS & Wire Up Real Values
- [x] 6.1 Bootstrap CDK
- [x] 6.2 Deploy pipeline stack
- [ ] 6.3 Trigger pipeline and wait for success
- [ ] 6.4 Get real CloudFormation outputs, update amplify-config.ts
- [ ] 6.5 Rebuild and redeploy frontend with real values
- [ ] 6.6 Seed DynamoDB with mystery data
- [ ] 6.7 Verify deployed site (curl 200, login page renders)

## PHASE 7: UI Redesign — Unique & Atmospheric
- [ ] 7.1 New globals.css with custom design system (fonts, colors, textures, animations)
- [ ] 7.2 Redesign login page (noir detective theme)
- [ ] 7.3 Redesign register page
- [ ] 7.4 Redesign sidebar & topbar (case file navigation)
- [ ] 7.5 Redesign dashboard page (mystery cards as case files)
- [ ] 7.6 Redesign history page (detective logbook)
- [ ] 7.7 Redesign play page & character modal (immersive atmosphere)
- [ ] 7.8 VERIFY: build + e2e tests pass
- [ ] 7.9 Deploy redesigned frontend, verify CloudFront

## PHASE 8: New Features — Incremental & Fully Tested
- [ ] 8.1 User Profile Page (detective ID card)
- [ ] 8.2 Mystery Detail Page (case briefing dossier)
- [ ] 8.3 Character Gallery During Play (polaroid cards)
- [ ] 8.4 Hint System (mysterious notes)
- [ ] 8.5 Game Timer & Score System (vintage clock)
- [ ] 8.6 Leaderboard (Hall of Fame)
- [ ] 8.7 Sound Effects & Ambient Audio (Web Audio API)
- [ ] 8.8 Multi-city Support — Barcelona Mystery
- [ ] 8.9 Achievement / Badge System (detective medals)
- [ ] 8.10 Progressive Web App (PWA)

## PHASE 9: Robustness & Polish
- [ ] 9.1 Error boundaries & fallback UI
- [ ] 9.2 Loading states & skeletons
- [ ] 9.3 Mobile responsiveness
- [ ] 9.4 Accessibility audit
- [ ] 9.5 Performance optimization
- [ ] 9.6 Final E2E test suite (40+ tests)
- [ ] 9.7 Final deploy & smoke test
